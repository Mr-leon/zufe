...
import org.smartjq.mvc.common.model.base.BaseSysUser;
...
public class SysUser extends BaseSysUser<SysUser> {
	...
	public Page<Record> getPage(int pnum,int psize, String orgid,String usernameSearch,String nameSearch){
		String sql  = " from sys_user u LEFT JOIN ec_organization o  on o.id = u.orgid  where 1=1 ";
	...
	public Page<Record> getPageByGroupId(int pnum,int psize, String groupid, String usernameSearch, String nameSearch) {
		String sql  = " from sys_user u LEFT JOIN sys_org o  on o.id = u.orgid, dj_mail_list_group_user as gu  where gu.userid=u.id ";
		if(!ShiroKit.getLoginUser().getUsername().equals("lion")){
			sql = sql + " and u.username !='"+Constants.SUUUUUUUUUUUUUPER_USER_NAME+"' ";
		}
		if(StrKit.notBlank(groupid)){
			sql = sql + " and gu.group_id='"+groupid+"' ";
		}
		if(StrKit.notBlank(usernameSearch)){
			sql = sql + " and u.username like '%"+usernameSearch+"%' ";
		}
