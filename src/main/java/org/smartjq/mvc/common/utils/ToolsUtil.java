...
public class ToolsUtil {
	private static final String SecretId = "AKID3q6F7idkrhaIq6JVfYAkDagU6SXLugCR";
	private static final String SecretKey = "6rLYZUGCqDNL2TpyzjBPN3TlqnSpzsOd";
	private static final String Region = "ap-shanghai";
	private static final String RegisterTemplateID = "623430";
	private static final String SmsSdkAppid = "1400379931";
	private static final String Sign = "拍贝科技";
	...
	public static String getRandom(int len) {
		...
		String number = "";
		for (int a = 0; a < len; a++) {
			number += rand.nextInt(10);// 生成6位数字
		}
		return number;

	}

	public static String sendRegisterSmsCode(String phone, String code, String time) {
		try {
			/*
			...
			req.setTemplateID(RegisterTemplateID);
			...
			JSONObject json = JSONObject.parseObject(SendSmsRequest.toJsonString(res));
			JSONArray arr = (JSONArray) json.get("SendStatusSet");
			JSONObject stat = arr.getJSONObject(0);
			return stat.getString("Code");
		} catch (Exception e) {
			logger.error("用户注册短信验证码发送失败", e);
		}
		return null;
	}
	
	public static String sendSms(String phone, String templateID, String[] templateParams) {
		try {
			/*
			 * 必要步骤： 实例化一个认证对象，入参需要传入腾讯云账户密钥对 secretId 和 secretKey
			 * 本示例采用从环境变量读取的方式，需要预先在环境变量中设置这两个值 您也可以直接在代码中写入密钥对，但需谨防泄露，不要将代码复制、上传或者分享给他人 CAM
...
//		File f = new File("C:\\Users\\Shang\\Desktop\\优惠活动\\弹窗+消费券-0410-OUT.ai");
//		System.out.println(isImage(f));
		String ph = "15400010001";
		System.out.println(isMobile(ph));
	}
